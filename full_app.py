...
# Explanation of NST
nst_explanation = """
Neural Style Transfer (NST) is a technique that uses deep neural networks to transform digital images or videos by applying the visual style of one image (style image) to the content of another (content image). It blends the content, such as the objects and layout from the content image, with the artistic style, like colors and textures from the style image, to create a new image.

NST works by minimizing a combination of content loss, which measures differences in neural activations between the content and generated images, and style loss, based on Gram matrices capturing correlations in filter responses. This process uses convolutional neural networks (CNNs), with the original method employing a pre-trained VGG-19 architecture. The total loss is iteratively reduced through backpropagation, updating the generated image until it matches both the content and style.

Key methods include the initial approach by Gatys et al. (2015), which used VGG-19, and later advancements like Google AI’s 2017 method for multiple styles and real-time style interpolation. Extensions include video stylization, faster processing with instance normalization, and feature transform methods like whitening and coloring transform for flexible style blending. NST is widely used in apps like DeepArt and Prisma for creating artistic images from photographs.
"""
...
def adain_app():
    ...
    if st.button("Start Stylization", key="adain_start"):
        if content_file and style_file:
            ...
            stylized_bytes = stylized.tobytes("raw", "RGB")
            st.download_button("Download Stylized Image", stylized_bytes, file_name="stylized.png", mime="image/png", key="adain_download")
        else:
...
def dumoulin_app():
    ...
    st.image(style_reference_image, caption="Style Indexes Reference", use_column_width=True)
    ...
    if st.button("Start Stylization", key="dumoulin_start"):
        if content_file:
            ...
            if not os.path.exists(model_path):
                ...
                st.image(stylized, caption="Stylized Image", use_column_width=True)
                stylized_bytes = stylized.tobytes("raw", "RGB")
                st.download_button("Download Stylized Image", stylized_bytes, file_name="stylized.png", mime="image/png", key="dumoulin_download")
        else:
...
def dumoulin_v2_app():
    ...
    if st.button("Start Stylization", key="v2_start"):
        if content_file and style_file:
            ...
            st.image(stylized, caption="Stylized Image", use_column_width=True)
            stylized_bytes = stylized.tobytes("raw", "RGB")
            st.download_button("Download Stylized Image", stylized_bytes, file_name="stylized.png", mime="image/png", key="v2_download")
        ...
        if content_file is not None and style1_file is not None and style2_file is not None:
            try:
                ...
                if not os.path.exists(checkpoint_path):
                    ...
                    st.image(stylized_denorm, caption="Stylized Image", use_column_width=True)
...
def stylize_johnson(content_img, model_path):
    ...
    model, _ , _ = load_model_johnson(model_path, model)
    model.eval()
...
def load_model_johnson(checkpoint_path, model, optimizer=None):
    checkpoint = torch.load(checkpoint_path, map_location=device)
...
# In dumoulin_app() function, update the image path if needed, e.g.:
style_reference_image = "style_index.png"  # Confirm this file exists in repo root

# In all back buttons, e.g. in dumoulin_app():
if st.button("Back to Main Page", key="dumoulin_back"):
    st.session_state.page = 'main'
    st.rerun()  # Updated from experimental_rerun

# Repeat for all other back buttons in other app functions
